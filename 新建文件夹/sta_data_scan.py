...
# coding:utf-8
"""
Name  : sta_data_scan.py
Author: xh
Time  : 2021/11/17 17:03
Desc  :
        1.open sta
...
def sta_data_scan():
    """connect ap,visit the website,and scan ap information"""
    #check open
    if wifi_connect_ap():
        ...
        #scan ap
        if wifi_scan():
            mars.Print("{}: sta scan pass".format(os.path.basename(__file__)))
            #close web
            ...
            mars.Print("{}: sta scan fail".format(os.path.basename(__file__)))
            mars.Verdict("fail", "sta scan fail")
    else:
